...
f = open("files/results/successful_match_" + date + ".csv", 'w+')
f.write("URL, year, make, model, price, valuation \n")
carsAdded = []
with open('files/urls.csv', 'r', newline='') as csvfile:
    reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
    counter = 0
    for row in reader:
        #myrtleURL = ', '.join(row)
        #print(myrtleURL)
        #driver.get(myrtleURL)

        currURL = ', '.join(row) + "/d/cars-trucks-by-owner/search/cto?postedToday=1&search_distance=300&sort=date"
        print(currURL)
        driver.get(', '.join(row) + "/d/cars-trucks-by-owner/search/cto?postedToday=1&search_distance=300&sort=date")

        containers = driver.find_elements_by_class_name("result-info")
        car_titles = driver.find_elements_by_class_name("result-title")
        #example = driver.find_element_by_xpath("//div[@id='a']//a[@class='click']")
        #car_prices = driver.find_elements_by_class_name("result-price")
#        car_links = driver.find_elements_by_css_selector('result-title hdrlnk a')
        for i in range(len(containers)):

            car_price = driver.find_element_by_xpath("//*[@id='search-results']/li[" + str(i+1) + "]/div/span[2]/span[1]")

            #print("Title is: \"" + car_titles[i].text + "\" \nPrice is: " + car_price.text + "\n\n")
            ...
            if make != "lexus" and (model == "ct" or model == "es" or model == "gs" or model == "hs" or model == "is" or model == "ls" or model == "gx" or model == "lx" or model == "nx" or model == "rx" or model == "lc" or model == "rc" or model == "sc"):
                model = ""
            elif make != "nissan" and model == "versa":
                model = ""
            ...
            if model == '' or make == '' or year == '' or price == "spam":
                price = "."
            elif int(year) <= 2022 and int(year) >= 2013:
                #print(year + " " + make + " " + model)
                #print(title)
                #print("price of this is " + price)
                #print(" ")
                priceList = []
                flag = 0
                if year == "2022" or year == "2021":
                    ...
                    yearStr = "files/to_sort/" + str(year)
                with open(yearStr + '.csv', 'r', newline='') as csvfile2:
                    reader2 = csv.reader(csvfile2, delimiter=' ', quotechar='|')
                    for row2 in reader2:
                        currRow = ', '.join(row2)
                        if model.upper() in currRow and year in currRow and make.upper() in currRow:
                            flag = 1
                            tempPrice = re.search(r'[^,]*$', str(row2[len(row2) - 1]))
                            if tempPrice:
                                valuation = tempPrice.group()
                                priceList.append(int(valuation))
                            else:
                                print("INVALID PRICE ERROR")

                if flag == 1:
                    print(title)
                    valuation = min(priceList)
                    if(int(price) > valuation):
                        print("Price greater than valuation by " + str(int(price) - valuation))
                    else:
                        print("Price is LOWER than valuation by " + str(valuation - int(price)))
                        print(price + " & valuation: $" + str(valuation))
                        print(currRow)
                        print("PRICE: $" + row2[0])
                        print(year + " " + make + " " + model)
                        copyExists = 0
                        carText = year + ", " + make + ", " + model + ", " + price + "," + str(valuation) + "\n"
                        if carsAdded:
                            for car in carsAdded:
                                if carText == car:
                                    copyExists = 1
                        if copyExists != 1:
                            f.write(currURL + ", " + carText)
                        carsAdded.append(carText)
                    print("\n\n")
